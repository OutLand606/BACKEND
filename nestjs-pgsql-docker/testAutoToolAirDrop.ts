...
//       xKuCoin(); // Gọi lại hàm để lặp lại chu kỳ
...
export const frogClickScript = () => {
  ...
  // Bắt đầu click trong 5 phút, sau đó nghỉ 27 phút
  function xKuCoin() {
    clickInterval = setInterval(clickFrog, 100); // Click mỗi 100ms

    // Dừng click sau 5 phút
    setTimeout(() => {
      clearInterval(clickInterval);
      console.log('Đã dừng click. Nghỉ ngơi trong 27 phút...');

      // Sau 27 phút, bắt đầu lại
      setTimeout(() => {
        console.log('Bắt đầu click lại...');
        xKuCoin(); // Lặp lại chu kỳ
      }, 1620000); // 27 phút
    }, 300000); // 5 phút
  }
...
export const yesCoinScript = () => {
  // Ép kiểu về HTMLCanvasElement và HTMLElement
  const canvasElement = document.querySelector(
    'canvas.sketch',
  ) as HTMLCanvasElement | null;
  const coinPoolElement = document.querySelector(
    '.coin-pool',
  ) as HTMLElement | null;

  if (!canvasElement) {
    console.log("Không tìm thấy phần tử 'canvas.sketch'.");
    return;
  }
  ...
  const stepSize = 5; // Di chuyển mỗi 5 pixel
  const interval = 10; // Thời gian giữa mỗi lần di chuyển
  ...
  // Hàm di chuyển chuột ngang qua canvas
  function simulateHorizontalMouseMove(
    cursorIndex: number,
    totalCursors: number,
  ) {
    const canvasWidth = canvasElement.width;
    ...
    const sectionHeight = Math.floor(canvasHeight / totalCursors);
    const startY = Math.floor(sectionHeight * cursorIndex); // Tọa độ Y của mỗi con trỏ
    ...
    function moveMouse() {
      ...
      x += stepSize;
      if (x >= canvasWidth) {
        x = 0; // Reset lại khi đạt giới hạn canvas
      }

      setTimeout(moveMouse, interval); // Tiếp tục di chuột
    }

    moveMouse(); // Bắt đầu di chuyển chuột
  }

  // Hàm kiểm tra giá trị của coin-pool và chạy con trỏ
  function checkCoinPoolAndMoveCursors() {
    const coinValue = parseInt(coinPoolElement.innerText, 10);

    if (coinValue === 3000) {
      console.log('Coin pool đầy, bắt đầu chạy con trỏ...');

      const totalCursors = 20; // Số lượng con trỏ
      for (let i = 0; i < totalCursors; i++) {
        simulateHorizontalMouseMove(i, totalCursors);
      }
    } else {
      console.log('Coin pool chưa đầy, chờ đến khi đầy 3000...');
      setTimeout(checkCoinPoolAndMoveCursors, 300000); // Kiểm tra lại sau 5 phút
    }
  }

  checkCoinPoolAndMoveCursors(); // Khởi động kiểm tra
};
